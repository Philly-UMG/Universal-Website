...
1. Clone the repo (GitHub Desktop or `git clone`), edit files in `umg_pages_bundle/`, commit, push to `main`. Done — Vercel redeploys automatically.
2. Small text fixes can be made directly on github.com (pencil icon on any file).
...
| What | Where | Notes |
|---|---|---|
| Code | GitHub org `Philly-UMG` | Owned by the `phillyumg` GitHub account |
| Hosting | Vercel "Philly UMG" account | Deploys from this repo; domain attached here |
| Domain (phillyumg.com) | Registered at Wix | DNS managed in the Wix dashboard; A record + www CNAME point at Vercel — don't change them |
| Google Ads tag | Google account | Tag ID `AW-18125632206` |
| Business email | PhillyUMG@gmail.com | Recovery/owner address for the accounts above |
